#python
#readme - v0.18 - Improved Waypoints
...
DRONE_SPEED = 2  # added this global variable for drone speed
timestamp = 0
...
def sysCall_actuation():
    ...
    #goal_pos = sim.getObjectPosition(goal_handle, -1)
    #goal_i, goal_j = get_grid_coordinates(goal_pos[:2])
    start_i,start_j,goal_i,goal_j = get_endpoints()
...
def follow_path(dst):
    ...
    current_pos_3D = sim.getObjectPosition(target_handle, -1)
    ...
    step_size = DRONE_SPEED * sim.getSimulationTimeStep()
    ...
    sim.setObjectPosition(target_handle, -1, next_pos)
    sim.setObjectPosition(quad_handle, -1, next_pos) #hack
...
def set_path(path):
    global line_handle
    line_handle = sim.addDrawingObject(sim.drawing_lines, 10, 0.0, -1, 9999, [0, 0, 1])
...
def add_start_object():
    # Define object sizes and positions
    object_size = 0.25 #get('cell_size')
    start_pos = get('start_pos')
    x,y,z = get_closest_grid_point(start_pos)

    # Add start object
    start_handle = sim.createPrimitiveShape(sim.primitiveshape_spheroid, [object_size, object_size, 0],0)
...
def add_goal_object():
    ...
    # Add goal object
    goal_handle = sim.createPrimitiveShape(sim.primitiveshape_spheroid, [object_size, object_size, 0],0)
