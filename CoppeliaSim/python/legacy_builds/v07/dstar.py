#python
#readme - v0.0.07 - add dstar path and update cells to be transparent
...
def set_path(path):
    lineHandle = sim.addDrawingObject(sim.drawing_lines, 6, 0.0, -1, 9999, [0, 0, 1])
    
    for curr, prev in zip(path[1:], path[:-1]):
        line = np.append(prev[0], curr[0])
        line = np.append(line, prev[1])
        line = np.append(line, curr[1])
        line = list(line)
        sim.addDrawingObjectItem(lineHandle, line)
...
def sysCall_actuation():
    ...
    for i in range(len(rx)-1):
        ...
        sim.addLog(0,'grid rows: {} grid cols: {}'.format(len(grid),len(grid[0])))
        sim.addLog(0,'rx:{} ry:{}'.format(rx[i],ry[i]))
        sim.addLog(0, 'current: {}'.format(current_cell_handle))
        sim.addLog(0,'rx:{} ry:{}'.format(rx[i+1],ry[i+1]))
        sim.addLog(0, 'next: {}'.format(next_cell_handle))        
        src_cell = sim.getObjectPosition(current_cell_handle, -1)
        dst_cell = sim.getObjectPosition(next_cell_handle, -1)
        sim.addLog(0,'src:{} dst:{}'.format(src_cell, dst_cell))
...
def add_grid_cell(i,j):
    ...
    sim.addLog(0, 'grid[{}][{}]={}'.format(i,j,cell_handle))
    return cell_handle
...
def init_grid():
    ...
    for i in range(nrows):
        for j in range(ncols):
            ...
            sim.addLog(0, 'init_grid -> grid[{}][{}]->{}: {}'.format(i,j,grid[i][j],sim.getObjectPosition(int(grid[i][j]),-1))) #TEST
    return grid
