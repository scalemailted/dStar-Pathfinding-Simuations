#python
#readme - v0.13 - Add a drone to the map
...
def add_drone():
    # Define object sizes and positions
    object_size = get('cell_size') / 2
    start_pos = get('start_pos')
    x,y,z = get_closest_grid_point(start_pos)

    # Add drone object
    drone_handle = sim.createDummy(object_size)
    sim.setObjectPosition(drone_handle, -1, (x,y,z) )
    sim.setObjectOrientation(drone_handle, -1, [0,0,0])

    # Set drone color to green
    #r,g,b = sim.getShapeColor(drone_handle, None, sim.colorcomponent_ambient_diffuse)
    #sim.setShapeColor(drone_handle, None, sim.colorcomponent_ambient_diffuse, [0, 1, 0])
