#python
#readme - v0.14 - Drone follows path -> works, but hacked together, slow, and crashes at goal
...
DRONE_SPEED = 0.02  # added this global variable for drone speed
timestamp = 0
...
def follow_path(dst):
    ...
    current_pos = np.array(next_pos[:2])
    
    # Update the start position for the next path finding run
...
def follow_path(path):
    ...
    # Iterate through the path and move the drone
    for i in range(path_index, len(path)):
        current, _ = path[i]
        target_pos = np.array(current[:2])
        direction = target_pos - current_pos
        distance = np.linalg.norm(direction)
        if distance > DRONE_SPEED:
            # Calculate the drone's next position
            direction /= distance
            next_pos = current_pos + DRONE_SPEED * direction
            next_pos = [next_pos[0], next_pos[1], start_pos[2]]
        else:
            # The drone has reached the target position
            next_pos = current
        # Set the new position of the drone
